...
3. **如果你拿到的路由器是别人用过的，请通电3分钟后长按RESET键至少15秒（对于极路由E30，RESET键在机器的底面）让路由器自动重启，这样做的目的是恢复出厂设置。如果你的机器没有RESET键或者固件未适配，请看[这里](#如何将路由器恢复出厂设置)的方法2或方法3恢复出厂设置**
...
- 方法1：
  - 在正常开机状态下长按RESET键至少15秒（对于极路由E30，RESET键在机器的底面）直到路由器自动重启，推荐采用此方法
