...
目前scut_padavan中使用的scutclient源码版本为v3.0或v3.1.2；scut_padavan有监测脚本处理认证失败的情况，无需任何额外配置，在客户端意外掉线后会自动重拨并记录到系统日志中。
...
截至最后更新日期，此款已经停产的路由器可以在二手市场以较低的价格(40-70元不等)购买到，是同价位中支持5GHz WiFi最便宜的路由器之一，其它同类机器有小米路由Mini和斐讯K2(40-80元不等)等，也是支持5GHz WiFi的低价路由器。你也可以选择scut_padavan项目支持的其它路由器，如DIR-882, K2P等MT7615+MT7621方案的路由器(200元以上,性能强大)，或者是NEWIFI3等MT7621矿渣路由器(千兆,100元全新,WiFi信号可能很差)，只要刷入scut_padavan即可按照本文档的后续内容进行配置。
...
5. 把电脑与路由器任意一个LAN口(对于极路由E30，就是四个**黄色**的LAN口之一)连接起来
   ...
   **对于Mac OS X，请百度如何设置有线网络为DHCP，或使用WiFi连接路由器，然后跳到第12步**

   **如果你只有一台手机，也可以用手机WiFi连接路由器，然后跳到第12步**
...
9. 把有线网卡的IP地址设置成自动获取，**如果你现在使用WiFi连接路由器则无需这一步操作**
    ...
    找到电脑的有线网卡，一般情况下它的名字为`以太网`，并且很可能带有`Realtek`之类的字符，如果你插好了网线并且路由器正常开机的话，该网卡的图标上就不会带有×号。右键单击你的有线网卡，选择`属性`
...
15. 查看scutclient中的日志，没有出现ERROR之类的输出说明认证成功，此时电脑应该可以正常上网了
...
对于动态IP(DHCP)，无需进行其它配置；对于静态IP，需要设置正确的IP地址、子网掩码、网关以及DNS服务器；对于PPPoE，需要设置PPPoE的用户名和密码。
...
极路由E30的串口波特率为`115200`，TTL焊盘位于FLASH的右上方，拆开机器后容易看到。
...
**不推荐购买AR7241+AR9287,AR9341这些过时的单频路由器（如tp-link 841n,941n等被某些校园网路由器奸商卖到上百元高价的路由器），推荐从QCA9558/QCA9563+QCA988X起步，也可以考虑IPQ401X系列，但是价格较贵。**
...
- 中兴E8820 (QCA9563+QCA9882，只有64M内存，FLASH是32M的，固件支持少)
