...
在SCUT校园网环境下，主要有以下3种方式实现让路由器下的设备使用IPv6
  ...
  1. NAT方法(推荐)
     ...
     - scut_padavan所有固件都已经集成NAPT66，按以下步骤配置即可：
        ...
        2. 进入`外部网络(WAN)->IPv6协议`，按照下图配置
            IPv6内网设置里面的`IPv6内网地址`，如果用`dc00:101:101::1`之类的保留地址，那么大部分设备会默认用IPv6去连接双栈接入的网站；如果用`fc00:101:101::1`之类的地址，那么大部分设备会默认用IPv4去连接双栈接入的网站，推荐使用`dc00:101:101::1`  
            ![](img/ipv6.png)
